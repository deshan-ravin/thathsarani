...
export const Vortex = (props: VortexProps) => {
  ...
  const yOff = 0.00125
  const zOff = 0.0005
  const backgroundColor = props.backgroundColor || "#18181b"
  let tick = 0
  ...
  const drawParticle = (
    ...
    const colorHue = life % 20 < 10 ? 45 : 0 // Alternate between yellow (45) and red (0)
    ctx.strokeStyle = `hsla(${colorHue},100%,60%,${fadeInOut(life, ttl)})`
    ctx.beginPath()
    ctx.moveTo(x, y)
    ctx.lineTo(x2, y2)
    ctx.stroke()
  ...
  return (
    <div className={cn("relative h-full w-full", props.containerClassName)}>
      ...
      {/* Add gradient fade overlay at the bottom */}
      <div
        className="absolute bottom-0 left-0 right-0 h-[30%] z-[1] pointer-events-none"
        style={{
          background: "linear-gradient(to top, rgb(24, 24, 27, 1) 0%, rgba(24, 24, 27, 0) 100%)",
        }}
      />

      {/* Add gradient fade overlay at the top */}
      <div
        className="absolute top-0 left-0 right-0 h-[30%] z-[1] pointer-events-none"
        style={{
          background: "linear-gradient(to bottom, rgb(24, 24, 27, 1) 0%, rgba(24, 24, 27, 0) 100%)",
        }}
      />
